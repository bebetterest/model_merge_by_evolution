...
class CEval:
    ...
    def run(self, shot: int, split: str, inf_rate: float):
        results, accs = {}, {}
        ...
        for task_name in self.TASK2DESC:
            ...
            result, acc = self.run_single_task(task_name, shot, split, inf_rate)
    ...
    @torch.no_grad()
    def run_single_task(self, task_name: str, shot: int, split: str, inf_rate: float):
        dataset = load_dataset(self.DATA_PATH, task_name)
        results = []
        acc = 0

        original_len = len(dataset[split])
        dataset[split] = dataset[split].select(range(max(1, int(inf_rate * original_len))))
        print(f"{task_name}: {len(dataset[split])}/{original_len}  (inf_rate: {inf_rate})")
